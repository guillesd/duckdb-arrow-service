from backend import (
    list_table_json_ser,
    get_table_stats_column_json_ser,
    get_table_stats_row_json_ser,
    get_table_stats_arrow_ser,
    get_nrows_json_ser,
    get_nrows_arrow_ser,
)
...
def get_table_stats_json(table_name: str, format: str = "row"):
    if format == "row":
        return Response(
            content=get_table_stats_row_json_ser(table_name),
            media_type="application/json",
        )
    elif format == "column":
        return Response(
            content=get_table_stats_column_json_ser(table_name),
            media_type="application/json",
        )
    else:
        return Response(
            content="Invalid format specified. Use 'row' or 'column'.",
...
def get_nrows_json(table_name: str, nrows: int = 10000):
    return Response(
        content=get_nrows_json_ser(table_name, nrows), media_type="application/json"
    )
