...
def get_table_stats_row_json_ser(table_name: str) -> str:
    ...
    result_dict = []
    for row in data:    
        row_dict = {column_names[i]: float(row[i]) if isinstance(row[i], Decimal) else row[i] for i in range(len(column_names))}
...
def get_table_stats_arrow_ser(table_name: str):
    """
